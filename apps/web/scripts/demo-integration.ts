...
async function main(): Promise<void> {
  ...
  const responseCount = decodeU64(extractReturnBytes(resCount.results, 0, "count_responses"))
  ...
  const rating = decodeOptionU64(extractReturnBytes(resRating.results, 0, "aggregate_rating"))
  if (rating === null) {
    console.log("aggregate_rating(form, []) → None (no rated responses yet)")
  } else {
  ...
  const verified = decodeBool(extractReturnBytes(resVerify.results, 0, "verify_response"))
  console.log(
